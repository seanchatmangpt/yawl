...
/**
 * Immutable record holding the result of a single {@link McpFmeaAnalyzer} analysis.
 *
 ...
 * <p>Status values:
 * <ul>
 *   <li>{@code "GREEN"} — no violations detected; the MCP subject passed all checks</li>
 *   <li>{@code "RED"}   — one or more violations detected; action required</li>
 * </ul>
 *
 * <p>Example usage:
 * <pre>{@code
 * McpFmeaReport report = analyzer.analyzeContext(context, providerCount);
 * if (!report.isClean()) {
 *     log.error("MCP FMEA {} (totalRpn={}): {}",
 *         report.status(), report.totalRpn(), report.violations());
 *     throw new SecurityException("MCP context failed FMEA checks");
 * }
 * }</pre>
 *
 ...
 * @since 6.0.0
 */
public record McpFmeaReport(
        Instant analyzedAt,
        List<McpFmeaViolation> violations) {

    /**
     * Compact canonical constructor — defensive copy + null validation.
     */
    public McpFmeaReport {
