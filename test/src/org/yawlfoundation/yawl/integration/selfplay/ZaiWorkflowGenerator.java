...
public class ZaiWorkflowGenerator {
    ...
    private org.yawlfoundation.yawl.integration.zai.ZaiService zaiService;
    ...
    public ZaiWorkflowGenerator(SelfPlayConfig config) {
        ...
        // Try to initialize ZaiService if API key is available
        if (apiKey != null && !apiKey.isEmpty()) {
            try {
                this.zaiService = new org.yawlfoundation.yawl.integration.zai.ZaiService(apiKey);
                this.zaiService.setSystemPrompt("You are a YAWL workflow expert. Generate only valid YAWL XML specifications.");
            } catch (Exception e) {
                logger.warning("Failed to initialize ZaiService: " + e.getMessage());
            }
        } else {
    ...
    public String generateWorkflow(String promptType, Map<String, String> params) {
        // First try using the ZaiService if available
        if (zaiService != null) {
            try {
                String prompt = buildPrompt(promptType, params);
                String response = zaiService.chat(prompt);
    ...
    public boolean isAvailable() {
        return (apiKey != null && !apiKey.isEmpty()) || zaiService != null;
    }
    ...
    public String getStatus() {
        if (zaiService != null) {
            return "configured - using ZaiService";
        }
        if (apiKey == null || apiKey.isEmpty()) {
