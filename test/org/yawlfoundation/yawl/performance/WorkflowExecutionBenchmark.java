...
public class WorkflowExecutionBenchmark {
    ...
    private static void runCaseCreationBenchmark() {
        ...
        int[] caseCounts = {100, 500, 1000, 5000};
    ...
    private static void runDataPersistenceBenchmark() {
        System.out.println("\n--- Data Persistence Benchmark ---");
        
        int[] recordCounts = {1000, 5000, 10000};
        
        for (int records : recordCounts) {
            long persistenceTime = testDataPersistence(records);
            double recordsPerSec = records / (persistenceTime / 1000.0);
            
            System.out.printf("Records: %,d | Total: %,dms | Throughput: %,.0f records/sec%n",
                records, persistenceTime, recordsPerSec);
        }
    }
    
    private static long testDataPersistence(int records) {
        long start = System.currentTimeMillis();
        ...
        for (int i = 0; i < records; i++) {
            ...
            executor.submit(() -> {
                try {
                    ...
                    DataPersistence.save("record-" + recordId, "data-" + recordId);
