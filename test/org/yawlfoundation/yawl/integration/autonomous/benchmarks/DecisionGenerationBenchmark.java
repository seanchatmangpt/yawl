...
import org.yawlfoundation.yawl.integration.orderfulfillment.DecisionWorkflow;
import org.yawlfoundation.yawl.integration.zai.ZaiService;

/**
 * Benchmark comparing concrete DecisionWorkflow vs generic ZaiDecisionReasoner.
 *
 * Measures latency and throughput for output generation.
 *
 * @author YAWL Foundation
 * @version 5.2
 */
public final class DecisionGenerationBenchmark {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("=== Decision Generation Benchmark ===");
        ...
        System.out.println("\n=== Concrete DecisionWorkflow (Baseline) ===");
        BenchmarkHarness concreteHarness = new BenchmarkHarness(
            "Concrete-DecisionWorkflow",
            10,
            100
        );
        ...
        BenchmarkHarness.BenchmarkResult concreteResult = concreteHarness.run(
            new BenchmarkHarness.BenchmarkOperation() {
                ...
                @Override
                public void setup() {
                    workflow = new DecisionWorkflow(zaiService);
                }

                @Override
                public void run() {
                    workflow.produceOutput(mockWorkItem);
                }
            }
        );

        System.out.println("\n=== Generic ZaiDecisionReasoner ===");
        ...
        BenchmarkHarness.BenchmarkResult genericResult = genericHarness.run(
            new BenchmarkHarness.BenchmarkOperation() {
                ...
                @Override
                public void run() {
                    reasoner.produceOutput(mockWorkItem);
                }
            }
        );
    ...
    private static WorkItemRecord createMockWorkItem() {
        ...
        wir.setID("test-workitem-2");
        wir.setCaseID("test-case-2");
