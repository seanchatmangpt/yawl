                ...
                {ok, DfgData} ->
                    ...
                    case file:write_file(OutputPath, OutputJson) of
                        ok ->
                            ...
                            io:format("Generated DFG summary: ~p~n", [DfgData]);
                        {error, WriteReason} ->
...
format_json_output(DfgData) ->
    {nodes, NodesList} = proplists:get_value(nodes, DfgData),
    {edges, EdgesList} = proplists:get_value(edges, DfgData),
    {statistics, StatsList} = proplists:get_value(statistics, DfgData),
    
    Nodes = lists:map(fun({id, Id}, {count, Count}) -> 
        io_lib:format("{\"id\": \"~s\", \"count\": ~p}", [Id, Count]) end, 
        lists:zip(NodesList, lists:nthtail(2, NodesList))),
    
    Edges = lists:map(fun({from, From}, {to, To}, {weight, Weight}) -> 
        io_lib:format("{\"from\": \"~s\", \"to\": \"~s\", \"weight\": ~p}", [From, To, Weight]) end, 
        lists:zip(lists:nthtail(2, EdgesList), lists:nthtail(5, EdgesList), lists:nthtail(8, EdgesList))),
    
    Stats = lists:map(fun({Key, Value}) -> 
        io_lib:format("\"~s\": ~p", [Key, Value]) end, StatsList),
    
    io_lib:format(
        "{\"nodes\": [~s], \"edges\": [~s], \"statistics\": {~s}}",
        [string:join(Nodes, ", "), string:join(Edges, ", "), string:join(Stats, ", ")]
    ).
