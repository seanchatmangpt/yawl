...
The output lists all 13 Maven modules. For each module you will see four fields that matter:
...
This ordering matters when you change a low-level module: every module that depends on it will recompile. The `-T 1.5C` flag lets Maven parallelize modules whose dependencies are already built.
