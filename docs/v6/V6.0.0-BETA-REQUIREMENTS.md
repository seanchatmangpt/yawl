...
|--------|--------|-------------|-----------|
| Case launch throughput | ≥500 cases/sec | JMH benchmark | 10% regression limit |
| Task completion latency | p99 ≤ 10ms | JMH benchmark | 10% regression limit |
| Work item retrieval | p99 ≤ 5ms | JMH benchmark | 10% regression limit |
...
| JVM heap under load | < 512MB | JVM metrics | Hard limit |
| Case monitor snapshot | < 100ms | API response time | Soft target |
