...
| Stage | Meaning | Primary Gaps from Previous Stage |
|-------|---------|----------------------------------|
| **Alpha** (current) | Core compiles; static analysis tools configured; 100% guard compliance | JaCoCo not run (0% measured); 28 files excluded in yawl-integration; A2A excluded entirely; SBOM configured but not executed; UnsupportedOperationException inventory unclassified |
| **Beta** (target) | JaCoCo run with real data ≥65%/55%; SBOM executed and clean; all deferred items named with owner + target version; static analysis profile run; exclusion inventory classified | Integration tests deferred to RC; performance baseline deferred to RC |
| **GA** (6.0.0) | All 7 quality gates pass; zero compiler exclusions; A2A and SPIFFE/MTLS active; full security hardening; benchmark regression suite green | None — full release |

### Alpha-to-Beta Delta: Five Critical Criteria

These five items are the hard gates between Alpha and Beta. Everything else is secondary.

| # | Criterion | Alpha State | Beta Required |
|---|-----------|-------------|---------------|
| Δ1 | JaCoCo coverage | `line_pct: 0` — no exec file generated | `mvn verify -P coverage` run; `line_pct ≥ 0.65`, `meets_line_target: true` |
| Δ2 | Deferred item inventory | "integration deferred" (vague) | Every excluded file in `yawl-integration/pom.xml` documented with named target (v6.0.1 or GA) and one-sentence reason |
| Δ3 | SBOM generated and scanned | CycloneDX plugin configured, not executed | `mvn cyclonedx:makeBom` run; `grype sbom` passes with 0 CVSS≥7 |
| Δ4 | Static analysis profile output | `spotbugs-findings.json` etc. show `"REPORTS_NOT_GENERATED"` | `mvn verify -P analysis` run; observatory refreshed with real output |
| Δ5 | UnsupportedOperationException audit | 45 files with UOE — category unknown | All UOE instances classified: (a) intentional private constructors, (b) design-deferred to v6.0.1, (c) incomplete requiring pre-GA work |

**If any of Δ1–Δ5 is not satisfied, Beta cannot be declared, regardless of other criteria.**
...
**In Beta** (requires compiler exclusions resolved):

MCP Server (`YawlMcpServer`):

> **Note**: `YawlMcpServer.java` is currently excluded from `yawl-integration/pom.xml`
> compilation due to MCP SDK v0.18.1 API mismatch. Beta requires this exclusion to be removed
> by upgrading `io.modelcontextprotocol.sdk` to `1.0.0-RC1` or later.

Once exclusion is resolved, MCP must satisfy:
- Transport: STDIO (primary); HTTP (secondary)
- **15 tools** covering case lifecycle, work item management, specification operations
  - Core 6 (from `facts/integration.json`): `launch_case`, `cancel_case`, `get_case_state`, `list_specifications`, `get_workitems`, `complete_workitem`
  - Additional 9: in `src/org/yawlfoundation/yawl/integration/mcp/`
- **6 resources**: 3 static (specs, cases, work items) + 3 parameterized
- **4 prompts**: workflow analysis, task completion guide, case troubleshooting, design review
- **3 completions**: auto-complete for specifications, work items, cases
- All tools and resources return structured JSON per MCP 2025-11-25 specification

Event Sourcing (basic):
- Case state change events published to event bus
- Work item state change events persisted for replay

**Deferred to v6.0.1** (excluded from compilation in Alpha; not in Beta):

| Feature | Current Exclusion Pattern | Reason | Target |
|---------|--------------------------|--------|--------|
| A2A Server (`YawlA2AServer`, `VirtualThreadYawlA2AServer`) | `**/a2a/**` (51 classes, entire package) | SDK `io.github.a2asdk:1.0.0.Alpha2` compilation errors | v6.0.1 |
| MCP Spring integration | `**/mcp/spring/**` | Spring MCP API mismatch | v6.0.1 |
| ZAI/GLM-4 (`ZaiFunctionService`) | `**/zai/ZaiFunctionService.java` | Requires Pm4Py (Python dep not in Maven cache) | v6.0.1 |
| OpenTelemetry config | `**/observability/OpenTelemetryConfig.java` | OTEL SDK internal API changes in 1.52.0 | v6.0.1 |
| Process mining | `**/processmining/**` | Pm4Py Python dependency | v6.0.1 |
| Order fulfillment | `**/orderfulfillment/**` | A2A dependency | v6.0.1 |
| Conflict resolution | `**/conflict/**` | A2A dependency | v6.0.1 |
| Event sourcing (advanced) | `**/eventsourcing/**` | Incomplete switch expressions | v6.0.1 |
| SPIFFE/MTLS | `**/spiffe/SpiffeEnabledZaiService.java` | SPIFFE workload identity infrastructure | v6.0.1 |
| Multi-module patterns | `**/multimodule/**` | A2A dependency | v6.0.1 |
| OAuth2 enforcement / RBAC | `**/oauth2/**` | Security review required | v6.0.1 |
| Gateway configs (AWS/Kong/Traefik) | Configuration only | Infrastructure-specific | v6.0.1 |
...
| yawl-worklet | Core | STABLE | Verify worklet service compiles | None |
