...
- **15 tools** covering case lifecycle, work item management, specification operations
  ...
  - Additional 9: verified in `src/org/yawlfoundation/yawl/integration/mcp/`
- **6 resources**: 3 static (specs, cases, work items) + 3 parameterized (case details, work item details, spec details)
...
- MCP SDK version: 1.0.0-RC1 or later (resolves current v0.18.1 API mismatch in pom.xml)

A2A Server (`YawlA2AServer` and `VirtualThreadYawlA2AServer`):
- HTTP REST transport; discovery card at `/.well-known/agent.json`
- Port: 8081
- **6 skills**: `launch_workflow`, `query_workflows`, `manage_workitems`, `cancel_workflow` + 2 additional (verified in `src/org/yawlfoundation/yawl/integration/a2a/`)
- `VirtualThreadYawlA2AServer`: virtual-thread variant — 1000 concurrent agents at ~1MB (vs 2GB with platform threads)
- Authentication: JWT Bearer (HS256), API Key (HMAC-SHA256), mTLS (SPIFFE X.509 SVID)
- Handoff protocol: JWT-based task delegation between agents
- All skills conform to A2A protocol specification
