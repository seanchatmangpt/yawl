...
/**
 * Immutable record holding the result of a single {@link A2AFmeaAnalyzer} analysis.
 *
 ...
 * <p>Status values:
 * <ul>
 *   <li>{@code "GREEN"} — no violations detected; the A2A subject passed all checks</li>
 ...
 * if (!report.isClean()) {
 *     log.error("A2A FMEA {} (totalRpn={}): {}",
 *         report.status(), report.totalRpn(), report.violations());
 *     throw new SecurityException("Agent failed A2A FMEA checks");
 * }
 * }</pre>
 *
 ...
 * @since 6.0.0
 */
public record A2AFmeaReport(
        Instant analyzedAt,
        List<A2AFmeaViolation> violations) {

    /**
     * Compact canonical constructor — defensive copy + null validation.
     */
    public A2AFmeaReport {
