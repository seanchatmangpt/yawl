 ...
 *
 * <p><b>Usage</b>: Records implementing this interface automatically satisfy all
 * accessor contracts by virtue of their component declarations.
 *
...
public sealed interface MarketplaceEvent
    ...
    /** @return the concrete event type (OrderCreated, OrderShipped, etc) */
    String eventType();

    /** @return unique identifier of the agent sending this event */
    String agentId();

    /** @return deduplication key (unique per agent per logical event) */
    String idempotencyToken();

    /** @return when the event occurred in the agent's system (UTC) */
    Instant timestamp();

    /** @return semantic version for backward compatibility */
    String version();

    /** @return optional contextual data (tracing, correlation IDs, etc); may be null */
     ...
     *
     * <p>Implementing records should call this from their compact constructors
     * to enforce the base field invariants.
     *
     * @param eventType the concrete event type
     * @param agentId the agent identifier
     * @param idempotencyToken the deduplication token
    ...
    static void validateBaseFields(String eventType, String agentId,
            String idempotencyToken, Instant timestamp, String version) {
