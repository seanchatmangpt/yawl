...
%% @doc Discover Declare constraints from OCEL.
-spec discover_oc_declare(reference()) -> {ok, list()} | {error, term()}.
discover_oc_declare(Handle) ->
    gen_server:call(?SERVER, {discover_oc_declare, Handle}).
...
handle_call({export_xes, Handle, Path}, _From, State) ->
    try
        case export_xes_nif(Handle, Path) of
            ok ->
                {reply, ok, State};
            {error, Reason} ->
                {reply, {error, Reason}, State}
...
handle_call({import_ocel_json, Path}, _From, State) ->
    try
        case import_ocel_json_direct(Path) of
            {ok, Handle} ->
...
handle_call({export_ocel_json, Handle, Path}, _From, State) ->
    try
        case export_ocel_json_nif(Handle, Path) of
            ok ->
                {reply, ok, State};
            {error, Reason} ->
                {reply, {error, Reason}, State}
...
%% @doc Import OCEL JSON from file path directly using NIF.
