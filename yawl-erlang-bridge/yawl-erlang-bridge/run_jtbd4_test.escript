...
main(_) ->
    ...
    lists:foreach(fun(App) ->
        case application:ensure_started(App) of
            ok -> io:format("Started ~p successfully~n", [App]);
            {error, Reason} -> io:format("Failed to start ~p: ~p~n", [App, Reason])
        end
    end, Apps),
    ...
    Result1 = process_mining_bridge:import_ocel_json_path(Input1),
    case Result1 of
        {ok, OcelId1} ->
    ...
    Result2 = process_mining_bridge:import_ocel_json_path(Input2),
    case Result2 of
        {ok, OcelId2} ->
