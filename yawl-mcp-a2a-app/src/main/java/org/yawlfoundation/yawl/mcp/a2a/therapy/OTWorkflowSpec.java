 ...
 *       ┣ progressScore &ge; 85.0 → OutcomeEvaluation → end
 *       ┗ progressScore &lt; 85.0  → PlanAdaptation → InterventionPlanning (loop)
 * </pre>
 *
 ...
 * <p>The YAWL XML spec is loaded and validated on every execution to guarantee
 * schema compliance. {@link OTSwarmCoordinator} implements the adaptation loop
 * programmatically via {@link OTSwarmConfig#maxAdaptationCycles()} rather than
 * relying on the YAWL XOR loop, ensuring deterministic termination in the
 * stateless engine context.</p>
 *
...
public final class OTWorkflowSpec {
     ...
     * to produce YAWL XML suitable for engine validation.</p>
     */
