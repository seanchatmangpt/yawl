...
if [[ "$PHASE" == "trend" ]]; then
    ...
    for file in $LATEST_FILES; do
        ...
        compile_ms=$(jq -r '.compile_ms // 0' "$file" 2>/dev/null || echo "0")
        test_ms=$(jq -r '.test_ms // 0' "$file" 2>/dev/null || echo "0")
        total_ms=$(jq -r '.total_ms // 0' "$file" 2>/dev/null || echo "0")
        status=$(jq -r '.status // "unknown"' "$file" 2>/dev/null || echo "unknown")

        compile_s=$(awk "BEGIN {printf \"%.1fs\", $compile_ms/1000}")
        test_s=$(awk "BEGIN {printf \"%.1fs\", $test_ms/1000}")
        total_s=$(awk "BEGIN {printf \"%.1fs\", $total_ms/1000}")
