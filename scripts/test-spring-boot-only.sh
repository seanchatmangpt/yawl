#!/bin/bash
# Test script for Spring Boot MCP-A2A application only

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

cd "$PROJECT_ROOT"

# Colors
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

log_info() { echo -e "${BLUE}[INFO]${NC} $1"; }
log_success() { echo -e "${GREEN}[PASS]${NC} $1"; }
log_error() { echo -e "${RED}[ERROR]${NC} $1"; }

echo "=== Spring Boot MCP-A2A Application Test ==="
echo

# Check if Docker is running
if ! docker info >/dev/null 2>&1; then
    log_error "Docker is not running. Please start Docker first."
    exit 1
fi

# Create network if needed
log_info "Creating yawl-network..."
docker network create yawl-network 2>/dev/null || true
log_success "Network ready"

# Build MCP-A2A image if needed
if ! docker image inspect yawl-mcp-a2a:6.0.0-alpha >/dev/null 2>&1; then
    log_info "Building MCP-A2A Docker image..."
    docker build -f docker/production/Dockerfile.mcp-a2a-app -t yawl-mcp-a2a:6.0.0-alpha . || {
        log_error "Failed to build MCP-A2A image"
        exit 1
    }
    log_success "MCP-A2A image built"
fi

# Start MCP-A2A service
log_info "Starting MCP-A2A service..."
docker compose -f docker-compose-simple-test.yml up -d yawl-mcp-a2a

# Wait for health check
log_info "Waiting for MCP-A2A to be healthy..."
timeout 60 bash -c "
    while ! docker compose -f docker-compose-simple-test.yml exec -T yawl-mcp-a2a curl -sf http://localhost:8080/actuator/health/liveness >/dev/null 2>&1; do
        sleep 5
        echo 'Waiting for MCP-A2A...'
    done
" || {
    log_error "MCP-A2A health check timeout"
    docker compose -f docker-compose-simple-test.yml logs yawl-mcp-a2a
    exit 1
}

log_success "MCP-A2A is healthy"

# Test endpoints
echo
log_info "Testing endpoints..."

# Test MCP health
log_info "Testing MCP health endpoint..."
if curl -s http://localhost:18081/mcp/health | jq -e '.status == "ok"' >/dev/null 2>&1; then
    log_success "MCP health check passed"
else
    log_error "MCP health check failed"
    curl -s http://localhost:18081/mcp/health | jq .
fi

# Test MCP tools
log_info "Testing MCP tools list..."
if curl -s -X POST http://localhost:18081/mcp \
    -H "Content-Type: application/json" \
    -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' | \
    jq -e '.result.tools | length > 0' >/dev/null 2>&1; then
    tool_count=$(curl -s -X POST http://localhost:18081/mcp \
        -H "Content-Type: application/json" \
        -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' | \
        jq '.result.tools | length')
    log_success "MCP tools available ($tool_count tools)"

    # List tools
    echo "Available tools:"
    curl -s -X POST http://localhost:18081/mcp \
        -H "Content-Type: application/json" \
        -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' | \
        jq '.result.tools[].name'
else
    log_error "MCP tools list failed"
fi

# Test A2A agent card
log_info "Testing A2A agent card..."
if curl -s http://localhost:18082/.well-known/agent.json | jq -e '.name' >/dev/null 2>&1; then
    agent_name=$(curl -s http://localhost:18082/.well-known/agent.json | jq -r '.name')
    agent_version=$(curl -s http://localhost:18082/.well-known/agent.json | jq -r '.version')
    log_success "A2A agent card available ($agent_name v$agent_version)"
else
    log_error "A2A agent card failed"
    curl -s http://localhost:18082/.well-known/agent.json | jq .
fi

# Test A2A health endpoint
log_info "Testing A2A health endpoint..."
if curl -s http://localhost:18082/a2a/health | jq -e '.status' >/dev/null 2>&1; then
    status=$(curl -s http://localhost:18082/a2a/health | jq -r '.status')
    log_success "A2A health endpoint ($status)"
else
    log_error "A2A health endpoint failed"
fi

# Test Spring Boot health
log_info "Testing Spring Boot health endpoints..."
curl -s http://localhost:18080/actuator/health/liveness | jq -r '.status' | xargs -I {} echo "Spring Boot Liveness: {}"
curl -s http://localhost:18080/actuator/health/readiness | jq -r '.status' | xargs -I {} echo "Spring Boot Readiness: {}"

echo
log_success "Spring Boot MCP-A2A application test completed successfully!"

# Cleanup
echo
log_info "Cleaning up..."
docker compose -f docker-compose-simple-test.yml down

echo
log_success "All tests passed! Spring Boot MCP-A2A application is working correctly."