...
main(_) ->
    ...
    %% Start dependencies
    Apps = [kernel, stdlib, mnesia, logger],
    [begin
    ...
    case application:ensure_started(process_mining_bridge) of
        ok ->
            io:format("✓ Process mining bridge application started~n");
        {error, AppReason} ->
            io:format("✗ Failed to start process mining bridge application: ~p~n", [AppReason]),
            halt(1)
    end,
