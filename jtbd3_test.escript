#!/usr/bin/env escript
%%! -pa ebin -pa /Users/sac/yawl/yawl-erlang-bridge/yawl-erlang-bridge/ebin -pa /Users/sac/yawl/yawl-erlang-bridge/yawl-erlang-bridge/test -pa /Users/sac/yawl/yawl-erlang-bridge/yawl-erlang-bridge/src
...
main(_) ->
    ...
    case process_mining_bridge:import_ocel_json_path(OcelPath) of
        ...
        {error, Error} ->
            io:format("✗ Failed to import OCEL: ~p~n", [Error]),
            halt(1)
    end,
    ...
    case process_mining_bridge:slim_link_ocel(OcelId) of
        ...
        {error, Error} ->
            io:format("✗ Failed to slim-link OCEL: ~p~n", [Error]),
            halt(1)
    end,
    ...
    case process_mining_bridge:discover_oc_declare(SlimId) of
        {ok, Constraints} ->
            ...
            if ConstraintCount > 0 ->
                FirstConstraint = hd(Constraints),
            ...
            if length(InvalidConstraints) =:= 0 ->
                ...
                io:format("Activity Names in Constraints: ~p~n", [Activities]);
            true ->
        ...
        {error, Error} ->
            io:format("✗ Failed to discover constraints: ~p~n", [Error]),
            halt(1)
    end,
