...
/**
 * Immutable training dataset for AutoML models.
 *
 * <p>Tabular format with feature vectors (rows) and corresponding labels.
 * Used for training ONNX models via TPOT2 and validating prediction accuracy.
 *
 * @param featureNames Column names (e.g., "caseDurationMs", "taskCount")
 * @param rows Feature vectors, one row per case
 * @param labels Outcome labels per row, same length as rows
 * @param specificationId Workflow specification or dataset identifier this data is extracted from
