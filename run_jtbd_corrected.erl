        ...
        {ok, InputContent} ->
            case jsx:decode(InputContent, [{return_maps, true}]) of
                #{<<"events">> := Events, <<"objects">> := Objects} ->
                    % Events is a list, Objects is a map
                    io:format("Input has ~p events, ~p objects~n", [length(Events), length(maps:values(Objects))]),
                        ...
                        {ok, OcelHandle} ->
                            ...
                            case catch process_mining_bridge:discover_dfg(OcelHandle) of
                                {ok, DfgJson} ->
                                    ...
                                    {ok, #{output => OutputPath, events => length(Events), objects => length(maps:values(Objects))}};
                                Error ->
                                    process_mining_bridge:free_handle(OcelHandle),
