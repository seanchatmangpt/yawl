...
public class YEnabledTransitionSet {
    ...
    public class TaskGroup {
        ...
        /**
         * Checks if this group represents a multi-instance task pattern.
         * This is determined by checking if there are multiple atomic tasks
         * in the group.
         */
        public boolean isMultiInstance() {
            return getAtomicTasks().size() > 1;
        }

        /**
         * Checks if any task in this group has conditions.
         * @return true if any task has conditions, false otherwise
         */
        public boolean hasConditions() {
            for (YAtomicTask task : getAtomicTasks()) {
        ...
        /**
         * Checks conditions for a specific task in this group.
         * @param task the task to check conditions for
         ...
         */
        public boolean checkConditions(YAtomicTask task, String caseID) throws YStateException {
            return task.checkConditions(caseID);
        }

        /**
         * Gets the split type for this group based on the number of tasks.
         * @return YTask._AND for parallel split, YTask._XOR for exclusive choice,
         *         YTask._OR for OR split
         */
        public int getSplitType() {
            int taskCount = getAtomicTasks().size();
            if (taskCount == 0) {
                return YTask._XOR; // Default for empty groups
            } else if (taskCount > 1) {
                // For multiple tasks, assume XOR unless specified otherwise
                return YTask._XOR;
            }
            return YTask._XOR; // Single task uses XOR semantics
        }
