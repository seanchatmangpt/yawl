...
public class YInternalCondition extends YNetElement implements YConditionInterface {
    ...
    public void remove(YIdentifier identifier, int amount) throws YStateException {
        _bag.remove(null, identifier, amount);
    }
    ...
    /**
     * Removes all the YIdentifiers from this condition.
     */
    public void removeAll() {
        _bag._idToQtyMap.clear();
    }
     ...
     * @throws YPersistenceException if there's a problem persisting the change.
     */
    public void removeOne(YPersistenceManager pmgr) throws YPersistenceException {
        YIdentifier id = getIdentifiers().get(0);
        try {
            _bag.remove(null, id, 1);
        } catch (YStateException e) {
            throw new YPersistenceException("Failed to remove identifier", e);
        }
    }

    /**
     * Remove one identifier equal to the specified identifier from the condition with persistence manager.
     * @param pmgr the persistence manager
