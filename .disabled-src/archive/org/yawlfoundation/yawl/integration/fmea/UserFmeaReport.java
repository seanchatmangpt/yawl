...
/**
 * Immutable record holding the result of a single {@link UserFmeaAnalyzer} analysis.
 *
 ...
 * <p>Status values:
 * <ul>
 *   <li>{@code "GREEN"} — no violations detected; the user subject passed all checks</li>
 ...
 * if (!report.isClean()) {
 *     log.error("User FMEA {} (totalRpn={}): {}",
 *         report.status(), report.totalRpn(), report.violations());
 *     throw new SecurityException("Principal failed FMEA checks");
 * }
 * }</pre>
 *
 ...
 * @since 6.0.0
 */
public record UserFmeaReport(
        Instant analyzedAt,
        List<UserFmeaViolation> violations) {

    /**
     * Compact canonical constructor — defensive copy + null validation.
     */
    public UserFmeaReport {
    ...
    public int totalRpn() {
        return violations.stream()
                .mapToInt(UserFmeaViolation::rpn)
                .sum();
    }
}
