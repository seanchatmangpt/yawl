 ...
 * - Engine cleanup operates only on the current thread's instance
 ...
 *
 * @see ThreadLocalYEngineManager#clearCurrentThread()
...
public class ThreadLocalYEngineManager {
     ...
     *
     * For now, thread-local isolation is achieved via manual cleanup and state management,
    ...
    private static YEngine createThreadLocalInstance(boolean persisting) {
        try {
            ...
            throw new RuntimeException("Thread-local engine creation failed", e);
        }
    }

    /**
     * Clears the YEngine instance for the current thread and removes the thread-local entry.
     *
     ...
     *
     * This method performs the same cleanup as YEngine.shutdown() but scoped to
     * the current thread when thread-local isolation is enabled.
     */
    public static void clearCurrentThread() {

        if (!ISOLATION_ENABLED) {
            // Backward compatible: noop (sequential mode uses global YEngine)
            return;
        }
        ...
        if (instance != null) {
            try {
                ...
                throw new RuntimeException("Failed to shutdown engine for thread " +
