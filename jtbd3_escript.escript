...
main(_) ->
    ...
    case process_mining_bridge:import_ocel_json(OcelPath) of
        {ok, OcelId} ->
            ...
            %% Step 2: Slim-link OCEL (if available)
            ...
            case erlang:function_exported(process_mining_bridge, slim_link_ocel, 1) of
                ...
                false ->
                    io:format("! slim_link_ocel not available, using original OCEL~n"),
                    UseId = OcelId
            end,
            
            %% Step 3: Discover OC-DECLARE constraints
            io:format("~nStep 3: Discovering OC-DECLARE constraints...~n"),
            case process_mining_bridge:discover_oc_declare(UseId) of
                {ok, Constraints} ->
                    ...
                    if UseId =:= OcelId ->
                        io:format("SLIM_ID: Not used (no slim_link_ocel)~n");
                    true ->
                        io:format("SLIM_ID: ~p~n", [UseId])
                    end,
