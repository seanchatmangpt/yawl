...
//!
//! OCEL Structure:
//! - OCELEvent: id, event_type, time, attributes, relationships
//! - OCELObject: id, object_type, attributes, relationships
//! - OCELRelationship: object_id, qualifier
...
use process_mining::{EventLog, OCEL, Importable};
use process_mining::core::event_data::object_centric::linked_ocel::{
    IndexLinkedOCEL, SlimLinkedOCEL, LinkedOCELAccess,
};
...
#[rustler::nif]
pub fn import_ocel_json_path(env: Env<'_>, path: String) -> NifResult<Term<'_>> {
    match OCEL::import_from_path(Path::new(&path)) {
        Ok(ocel) => {
            let id = store_item(RegistryItem::OCEL(ocel));
            Ok((ok(), id).encode(env))
        }
        Err(e) => Ok((error(), format!("Failed to import OCEL: {}", e)).encode(env))
    }
}

#[rustler::nif]
pub fn import_xes_path(env: Env<'_>, path: String) -> NifResult<Term<'_>> {
    match EventLog::import_from_path(Path::new(&path)) {
        Ok(log) => {
            let id = store_item(RegistryItem::EventLog(log));
            Ok((ok(), id).encode(env))
        }
        Err(e) => Ok((error(), format!("Failed to import XES: {}", e)).encode(env))
    }
}

#[rustler::nif]
pub fn num_events(env: Env<'_>, id: String) -> NifResult<Term<'_>> {
    let registry = REGISTRY.lock().unwrap();
    let item = registry.get(&id)
        .ok_or_else(|| rustler::Error::Term(Box::new(format!("Not found: {}", id))))?;
    match item {
        RegistryItem::OCEL(ocel) => Ok((ok(), ocel.events.len()).encode(env)),
        RegistryItem::IndexLinkedOCEL(locel) => Ok((ok(), locel.get_num_evs()).encode(env)),
        RegistryItem::SlimLinkedOCEL(locel) => Ok((ok(), locel.get_num_evs()).encode(env)),
        _ => Ok((error(), "Not an OCEL").encode(env)),
    }
}

#[rustler::nif]
pub fn num_objects(env: Env<'_>, id: String) -> NifResult<Term<'_>> {
    let registry = REGISTRY.lock().unwrap();
    let item = registry.get(&id)
        .ok_or_else(|| rustler::Error::Term(Box::new(format!("Not found: {}", id))))?;
    match item {
        RegistryItem::OCEL(ocel) => Ok((ok(), ocel.objects.len()).encode(env)),
        RegistryItem::IndexLinkedOCEL(locel) => Ok((ok(), locel.get_num_obs()).encode(env)),
        RegistryItem::SlimLinkedOCEL(locel) => Ok((ok(), locel.get_num_obs()).encode(env)),
        _ => Ok((error(), "Not an OCEL").encode(env)),
    }
}

#[rustler::nif]
pub fn index_link_ocel(env: Env<'_>, id: String) -> NifResult<Term<'_>> {
    let registry = REGISTRY.lock().unwrap();
    let item = registry.get(&id)
        .ok_or_else(|| rustler::Error::Term(Box::new(format!("Not found: {}", id))))?;
    match item {
        RegistryItem::OCEL(ocel) => {
            ...
            drop(registry);
            let new_id = store_item(RegistryItem::IndexLinkedOCEL(linked));
            Ok((ok(), new_id).encode(env))
        }
        _ => Ok((error(), "Not an OCEL").encode(env)),
    }
}
...
#[rustler::nif]
pub fn slim_link_ocel(env: Env<'_>, id: String) -> NifResult<Term<'_>> {
    let registry = REGISTRY.lock().unwrap();
    let item = registry.get(&id)
        .ok_or_else(|| rustler::Error::Term(Box::new(format!("Not found: {}", id))))?;
    match item {
        RegistryItem::OCEL(ocel) => {
            ...
            drop(registry);
            let new_id = store_item(RegistryItem::SlimLinkedOCEL(linked));
            Ok((ok(), new_id).encode(env))
        }
        _ => Ok((error(), "Not an OCEL").encode(env)),
    }
...
#[rustler::nif]
pub fn ocel_type_stats(env: Env<'_>, id: String) -> NifResult<Term<'_>> {
    let registry = REGISTRY.lock().unwrap();
    let item = registry.get(&id)
        .ok_or_else(|| rustler::Error::Term(Box::new(format!("Not found: {}", id))))?;
    match item {
        RegistryItem::OCEL(ocel) => {
            ...
            for o in &ocel.objects {
                *obj_types.entry(o.object_type.clone()).or_insert(0) += 1;
            }
            let json = serde_json::json!({"event_type_counts": event_types, "object_type_counts": obj_types}).to_string();
            Ok((ok(), json).encode(env))
        }
        _ => Ok((error(), "Not an OCEL").encode(env)),
    }
...
// DFG Discovery
#[rustler::nif]
pub fn discover_dfg<'a>(env: Env<'a>, id: String) -> NifResult<Term<'a>> {
    let registry = REGISTRY.lock().unwrap();
    let item = registry.get(&id)
        .ok_or_else(|| rustler::Error::Term(Box::new(format!("Not found: {}", id))))?;
    let ocel = match item {
        RegistryItem::OCEL(ocel) => ocel,
        _ => return Ok((error(), "Not an OCEL").encode(env)),
    };

    // Build traces: group events by object ID using relationships
    ...
    for (_, mut evs) in traces {
        ...
        let acts: Vec<_> = evs.iter().map(|e| e.event_type.clone()).collect();
        if !acts.is_empty() {
    ...
    let dfg_json = serde_json::json!({"nodes": nodes, "edges": e, "start_activities": starts, "end_activities": ends}).to_string();
    Ok((ok(), dfg_json).encode(env))
}

// Petri Net Discovery
#[rustler::nif]
pub fn discover_petri_net<'a>(env: Env<'a>, id: String) -> NifResult<Term<'a>> {
    let registry = REGISTRY.lock().unwrap();
    let item = registry.get(&id)
        .ok_or_else(|| rustler::Error::Term(Box::new(format!("Not found: {}", id))))?;
    let ocel = match item {
    ...
    // Build traces from OCEL using relationships
    let mut traces: HashMap<String, Vec<_>> = HashMap::new();
    for e in &ocel.events {
        for r in &e.relationships {
            traces.entry(r.object_id.clone()).or_default().push(e);
    ...
    for (_, mut evs) in traces {
        if evs.is_empty() { continue; }
        evs.sort_by(|a, b| a.time.cmp(&b.time));
        let t: Vec<_> = evs.iter().map(|e| e.event_type.clone()).collect();
    ...
    let transitions: Vec<_> = activities.iter().map(|a| serde_json::json!({"id": format!("t_{}", a), "name": a})).collect();
