...
class RequestReplyMarathon extends MarathonTestBase {
    ...
    void requestReplyMemoryLeakDetection() throws Exception {
        ...
        for (int i = 0; i < NUM_RESPONDERS; i++) {
            ActorRef responder = runtime.spawn(self -> respondToQueries());
     ...
     */
    private void respondToQueries() throws Exception {
    ...
    private void fireAsyncRequest(ActorRef responder, long correlationId) {
        ActorRef self = ActorRuntime.current().self();
        ...
        try {
            ...
            CompletableFuture<Object> future = responder.ask(query, Duration.ofMillis(REQUEST_TIMEOUT_MS));
