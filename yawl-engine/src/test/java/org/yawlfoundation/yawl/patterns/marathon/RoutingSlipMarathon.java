...
class RoutingSlipMarathon extends MarathonTestBase {
    ...
    private static final int TEST_DURATION_MINUTES = 240;  // 4 hours
    ...
    @Test
    @Timeout(minutes = 310)  // 4 hours + 50 min buffer
    ...
    void routingSlipCaseCleanupMarathon() throws Exception {
        ...
        for (int i = 0; i < NUM_ROUTING_ACTORS; i++) {
            ...
            ActorRef actor = runtime.spawn(self -> routeCase(actorIndex));
     ...
     */
    private void routeCase(int actorIndex) throws Exception {
        ActorRuntime runtime = ActorRuntime.current();

        while (testRunning.get() && !Thread.currentThread().isInterrupted()) {
            Object msg = runtime.receiveMessage();
