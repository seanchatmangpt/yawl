...
public class RequestReplyBenchmark extends BenchmarkBase {
        ...
        for (int i = 0; i < 1000; i++) {
            ActorRef responder = runtime.spawn(mailbox -> {
                ...
                if (msg instanceof String request) {
                    responder.send(request + "_replied");
                }
            });
    ...
    public void askThroughput(int concurrency, Blackhole bh) throws Exception {
        ...
        for (int i = 0; i < concurrency; i++) {
            ...
            CompletableFuture<Object> reply = requestReply.ask(
                "test_message_" + i,
    ...
    public void timeoutCleanupRate(Blackhole bh) throws Exception {
        ...
        for (int i = 0; i < 100_000; i++) {
            requestReply.ask(
                "timeout_message_" + i,
                java.time.Duration.ofMillis(50)  // Short timeout to trigger cleanup
            );
        }
