...
# OCEL output state
OCEL_COUNT=$(find "${PROJECT_DIR}/sim-output" -name "pi-*.ocel" 2>/dev/null | wc -l | tr -d ' ')
echo "PI OCEL files in sim-output/: ${OCEL_COUNT:-0}"
if [ "${OCEL_COUNT:-0}" -eq 0 ]; then
  echo "  WARNING: YawlSimulator.runPI() has never completed"
fi

# Generated TTL state
TTL_COUNT=$(find "${PROJECT_DIR}" -name "*.ttl" -path "*/generated/*" 2>/dev/null | wc -l | tr -d ' ')
echo "Generated TTL files: ${TTL_COUNT:-0}"
